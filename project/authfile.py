...
def login_post():
    ...
    if not user or not check_password_hash(user.password, password):
        ...
        return redirect(url_for('webpage/auth.login')) # if the user doesn't exist or password is wrong, reload the page

    # if the above check passes, then we know the user has the right credentials
    login_user(user, False)
    return redirect(url_for('webpage/main.profile'))
...
def signup_post():
    ...
    if user: # if a user is found, we want to redirect back to signup page so user can try again
        ...
        return redirect(url_for('webpage/auth.signup'))
    ...
    db.session.commit()
    return redirect(url_for('webpage/auth.login'))
